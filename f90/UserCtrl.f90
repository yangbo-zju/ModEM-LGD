     ...
     select case (job)
      ...
      case (FORWARD) !F
        if (narg < 3) then
           ...
           write(0,*)
           write(0,*) 'Number of cuBICG iters per divergence correction : 40'
           ...
           write(0,*)
           write(0,*) 'Forward solver method PCG|QMR|TFQMR|cuBICG      : cuBICG'
           stop
        else
	       ctrl%rFile_Model = temp(1)
	       ctrl%rFile_Data = temp(2)
      ...
      case (INVERSE) ! I
        if (narg < 3) then
           ...
           case ('NLCG','DCG','Hybrid','LBFGS','LGD')
              	! write(0,*) 'Inverse search ',trim(ctrl%search),' selected.'
           case default
		write(0,*) 'Unknown inverse search. Usage: -I [NLCG | DCG | Hybrid | LBFGS | LGD]'
		stop
           end select
