...
interface UpperLowerBounds
   MODULE PROCEDURE UpperLowerBounds_modelParam
end interface

interface SqrtModel
   MODULE PROCEDURE Sqrt_modelParam
end interface

interface ElementWiseMult
   MODULE PROCEDURE ElementWiseMult_modelParam
end interface
...
!**********************************************************************
   ...
   subroutine linComb_modelParam(a1,m1,a2,m2,m)
     ...
     if((m1%Ny .ne. m2%Ny).or. (m1%Nx .ne. m2%Nx) .or. &
		(m1%NzEarth .ne. m2%NzEarth)) then
        ! write(*,*),m1%Ny,m2%Ny,m1%Nx,m2%Nx,m1%NzEarth,m2%NzEarth
   ...
   !**********************************************************************
   subroutine UpperLowerBounds_modelParam(m_u, m_l, m)
  ! Ensure that the model's data stays within the given range

   type(modelParam_t), intent(inout) :: m
   real(kind=prec), intent(in) :: m_u, m_l
   ...
   do k = 1,m%NzEarth
      do j = 1,m%Ny
         do i = 1,m%Nx
            m%cellCond%v(i,j,k) = max(min(m%cellCond%v(i,j,k), m_u), m_l)
         end do
      enddo
   enddo

   end subroutine UpperLowerBounds_modelParam
  
   !**********************************************************************
   subroutine Sqrt_modelParam(m)
  ! compute square root of model
   ...
   do k = 1,m%NzEarth
      do j = 1,m%Ny
         do i = 1,m%Nx
            m%cellCond%v(i,j,k) = sqrt(m%cellCond%v(i,j,k))
         end do
      enddo
   enddo

   end subroutine Sqrt_modelParam

   !**********************************************************************
   subroutine ElementWiseMult_modelParam(m)
  ! compute element wise multiplication
   ...
   do k = 1,m%NzEarth
      do j = 1,m%Ny
         do i = 1,m%Nx
            m%cellCond%v(i,j,k) = m%cellCond%v(i,j,k) ** 2
         end do
      enddo
   enddo

   end subroutine ElementWiseMult_modelParam
